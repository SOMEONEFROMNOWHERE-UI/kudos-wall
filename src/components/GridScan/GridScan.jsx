...
export const GridScan = ({
  ...
  useEffect(() => {
    ...
    let visible = true;
    const visObs = new IntersectionObserver(([entry]) => { visible = entry.isIntersecting; }, { threshold: 0 });
    visObs.observe(container);

    let last = performance.now();
    let frameSkip = 0;
    const SKIP_FRAMES = isMobile ? 1 : 0; // render every 2nd frame on mobile
        ...
        if (ts - lastDetect >= 100) {
          ...
          try {
            ...
            if (res?.detection) {
              ...
              lookTarget.current.set(Math.tanh(nx), Math.tanh(ny));
