...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  // Professional Iridescent/Rainbow gradient for the scan beam (shifting slowly through 3D space)
  vec3 iridescent = 0.5 + 0.5 * cos(iTime * 0.8 + hit.z * 0.4 + hit.x * 0.3 + vec3(0.0, 2.0, 4.0));
  
  // Mix the base uScanColor with the iridescent effect for a highly professional foil look
  vec3 scanCol = mix(uScanColor, iridescent, 0.85) * combinedPulse;
  vec3 scanAura = mix(uScanColor, iridescent, 0.85) * combinedAura;
